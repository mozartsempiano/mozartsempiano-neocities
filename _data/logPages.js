...
const TMDB_BACKDROP_CACHE_PATH = path.join(
	process.cwd(),
	".cache",
	"tmdb-backdrops.json",
);
...
const LOG_DYNAMIC_HEADER_SLUGS = new Set([
	"cinema-log",
	"series-log",
	"anime-log",
]);
...
const TMDB_NOT_FOUND_CACHE_DAYS = toPositiveNumber(
	process.env.TMDB_NOT_FOUND_CACHE_DAYS,
	7,
);
...
function isEntryFresh(entry, forceRefresh, cacheDays, notFoundCacheDays) {
	...
	const maxAgeDays =
		entry.status === "not_found" ? notFoundCacheDays : cacheDays;
	return ageMs <= maxAgeDays * 24 * 60 * 60 * 1000;
}

async function fetchTmdbBackdropEntry(
	title,
	year,
	mediaType,
	tmdbKey,
	...
	const result =
		results.find((item) => item && item.backdrop_path) || results[0];
...
async function resolveTmdbBackdropEntry(
	title,
	year,
	mediaType,
	tmdbKey,
	...
	const hasYear = Number.isFinite(
		Number.parseInt(String(year || "").trim(), 10),
	);

	const job = (async () => {
		try {
			let next = await fetchTmdbBackdropEntry(
				title,
				year,
				safeMediaType,
				tmdbKey,
			...
			if (next?.status === "not_found" && hasYear) {
				next = await fetchTmdbBackdropEntry(
					title,
					year,
					safeMediaType,
					tmdbKey,
...
async function resolveTmdbBackdropWithFallback(
	title,
	year,
	mediaType,
	tmdbKey,
	...
	for (const currentType of mediaTypes) {
		const entry = await resolveTmdbBackdropEntry(
			title,
			year,
			currentType,
			tmdbKey,
...
function normalizePosterProvider(value) {
	...
	if (["open-library", "open_library", "ol"].includes(normalized))
		return "openlibrary";
	return null;
}
...
function loadSourceMeta(slug) {
	...
	const posterProviderOverride = normalizePosterProvider(
		parsed.data.posterProvider,
	);
	const tmdbMediaTypeOverride = normalizeTmdbMediaType(
		parsed.data.tmdbMediaType,
	);
	const enableTmdbPostersOverride = parseBooleanLike(
		parsed.data.enableTmdbPosters,
	);
	const legacyEnablePostersOverride = parseBooleanLike(
		parsed.data.enablePosters,
	);
...
module.exports = (async () => {
	...
	const logHeaderImageMode = normalizeLogHeaderImageMode(
		siteData.logHeaderImageMode,
	);
	const tmdbKey = process.env.TMDB_API_KEY;
	const isServeMode = process.env.ELEVENTY_RUN_MODE === "serve";
	const tmdbDisabledInServe =
		process.env.ELEVENTY_DISABLE_TMDB_IN_SERVE === "1";
	...
	for (const fileName of files) {
		...
		for (let index = 0; index < years.length; index += 1) {
			...
			const newerUrl = newerYear
				? newerYear === newestYear
					? `/${slug}/`
					: `/${slug}/${newerYear}/`
				: null;
			...
			pages.push({
				...
				imgPrincipalCaption:
					dynamicHeader.imgPrincipalCaption ||
					fallbackHeader.imgPrincipalCaption,
